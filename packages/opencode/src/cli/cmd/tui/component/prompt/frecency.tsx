...
import { readText, writeFile as writeFileUtil } from "@/util/fs-extra"
...
export const { use: useFrecency, provider: FrecencyProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    onMount(async () => {
      const text = await readText(frecencyFilePath).catch(() => "")
      const lines = text
        .split("\n")
        .filter(Boolean)
        .map((line) => {
      ...
      if (sorted.length > 0) {
        ...
        writeFileUtil(frecencyFilePath, content).catch(() => {})
      }
    })
